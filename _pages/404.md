...
<style>
  ...
  #snakeCanvas {
    /* Removed background-color to avoid adding a black background */
    border-radius: 15px;
    margin-top: 10px;
    width: 500px;
    height: 500px;
    cursor: pointer;
    background-color: #fff; /* Optional: Set to white or any desired color for better visibility */
  }
  ...
  #overlayText {
    ...
    color: #000; /* Changed to black for visibility on a white canvas */
    font-size: 28px;
    font-family: Arial, sans-serif;
    text-align: center;
    pointer-events: none;
  ...
  #score {
    margin-top: 10px;
    font-size: 20px;
    color: #fff; /* Adjust color if needed based on the canvas background */
  }
...
<script>
  ...
  if (!isMobileDevice()) {
    ...
    function drawGame() {
      ...
      // Draw the food
      ctx.fillStyle = "#f00"; // Red food for visibility
